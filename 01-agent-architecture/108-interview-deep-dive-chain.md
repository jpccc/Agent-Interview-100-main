...
面试追问链（Interview Deep-Dive Chain）是一种结构化面试技术：面试官从基础问题出发，根据候选人回答逐层深入，最终触达系统设计层面。

**在 AI Agent 领域的典型追问路径：** 从“什么是 LLM Agent”起步，经过 Agent Loop → 终止条件 → 错误恢复 → Self-Reflection → Reflection 局限性 → RAG 集成 → Multi-Agent 协作 → 生产部署，最终落到安全与 Guardrails。

**核心价值：** 不在于“难倒候选人”，而在于 **找到候选人的能力天花板并评估其思维深度**。
