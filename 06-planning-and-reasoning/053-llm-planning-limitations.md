...
LLM 作为规划器面临根本性挑战。Kambhampati (ASU, 2024) 的核心论点是 **"LLM 不能真正规划，但可以在规划中发挥建设性作用"**。

**主要局限：**

1. **约束满足能力弱** — LLM 难以同时满足多个相互制约的条件
2. **长程规划退化** — 随着步骤增多，累积错误导致计划质量急剧下降
3. **环境模型缺失** — LLM 没有真正的世界模型，无法准确预测动作的后果
4. **幻觉导致无效计划** — 生成看似合理但实际不可执行的步骤

**缓解方案：** 核心思路是 **LLM-Modulo 框架**——LLM 生成候选计划，外部验证器检查正确性，不合格则反馈修正。

**实证数据：** 在 Blocksworld 基准上，纯 LLM 规划仅 30-40% 可行率，而 LLM-Modulo 框架可将可行率提升到 80%+。
...
| 局限 | 问题 | 典型示例 | 根因 |
|------|------|----------|------|
| **约束满足** | LLM 难以同时满足多个约束条件 | 安排5人会议，每人有不同的空闲时间段 → LLM 经常忽略部分约束 | 自回归生成逐 token 产生，无法全局优化 |
| **长程规划退化** | 步骤越多，计划质量越差 | 10 步计划的可行率远低于 3 步计划 | 每步的小错误会累积放大 |
| **世界模型缺失** | LLM 无法准确模拟动作的后果 | 移动积木 A 到 B 上 → LLM 可能忘记 A 原来下面的积木会暴露出来 | LLM 是语言模型，不是物理/逻辑模拟器 |
| **幻觉与虚构** | 生成看似合理但不可执行的步骤 | 计划中引用不存在的 API、使用不可用的工具 | LLM 优化的是语言流畅度，不是计划可行性 |
| **回溯能力缺失** | 标准自回归 LLM 无法回头修改已生成的步骤 | 发现第 3 步需要第 1 步的不同输出时，无法物理改写第 1 步 | 单向生成架构的根本限制——已写出的 token 进入 KV-cache 即不可修改 |

> **重要例外：** Reasoning Models（o1/o3、DeepSeek-R1、Claude extended thinking 等）通过 RL 训练涌现出 **功能性回溯**——在思考链内部说 "wait, let me reconsider..." 并重新走一条推理路径。本质仍是顺序生成，但语义层面已能模拟回溯效果。参见 055-reasoning-models.md 的 Aha moment 部分。
...
> **传统规划器是什么？**
>
> 指基于**形式化逻辑和搜索算法**的经典 AI 规划系统，典型代表包括 Fast Downward、SATPlan、GraphPlan，使用 PDDL（Planning Domain Definition Language）精确定义状态、动作、前提条件和效果。规划器在状态空间中穷举或启发式搜索，数学上保证每步满足约束，因此可行率和最优率都是 100%。
>
> **与 LLM 规划的核心差异：**
>
> | 维度 | 传统规划器 | LLM 规划 |
> |------|-----------|----------|
> | **输入** | 形式化定义（状态、动作、前提条件、效果） | 自然语言描述 |
> | **推理方式** | 精确的逻辑推理 + 搜索 | 概率性的语言生成 |
> | **约束满足** | 100%（数学保证） | 经常遗漏 |
> | **最优性** | 可证明最优 | 无法保证 |
> | **灵活性** | 低（需要人工形式化建模） | 高（直接理解自然语言） |
>
> **为什么还需要 LLM？** 传统规划器的瓶颈是建模成本极高——需要人类专家把现实世界问题手动翻译成 PDDL 形式化描述。而 LLM 的优势在于能直接理解自然语言需求、处理模糊/开放性任务。LLM-Modulo 框架正是结合了两者优点：LLM 负责理解和生成候选方案，传统验证器负责保证正确性。
...
| 研究 | 核心发现 |
|------|----------|
| **Kambhampati 2024** | LLMs cannot plan but can help planning. LLM-Modulo 框架让 LLM 作为候选生成器，外部验证器保证正确性 |
| **Valmeekam et al. 2023** | 在 Blocksworld 上，GPT-4 直接生成的计划仅有约 35% 可行，远低于人们的预期 |
| **DeepPlanning 2026** | 隐含约束比显式约束更难被 LLM 检测到。环境中未明确说明的限制是 LLM 规划的最大盲区 |
| **PlanGenLLMs Survey** | LLM 在需要精确状态追踪的规划任务上表现最差，但在需要常识推理的规划任务上有独特优势 |
