...
Monte Carlo Tree Search (MCTS) 是一种结合树搜索与随机模拟的决策算法，通过四步循环——**选择（Selection）、扩展（Expansion）、模拟（Simulation）、反向传播（Backpropagation）**——在庞大的决策空间中找到近似最优解。

**在 Agent 规划中的价值：** MCTS 将 LLM 的推理能力与系统化搜索结合，克服了 LLM 单次生成无法回溯的根本局限。

**代表性框架 LATS（Language Agent Tree Search, ICML 2024）：** 将 MCTS 应用于 LLM Agent，用 LLM 同时充当多个角色——**决策、生成、评估、反思**（其中 Self-Reflection 是核心创新），在 HumanEval 编程任务上达到 94.4%（GPT-4），超越了所有已知的 prompting 方法。

**核心优势：** 探索-利用平衡——既深入挖掘有前景的方案，又不忽略潜在的好路径。
...
| 策略 | 搜索方式 | 优势 | 劣势 | LLM 调用 |
|------|----------|------|------|----------|
| **Greedy (CoT)** | 单路径，无回溯 | 最快、最便宜 | 容易卡在局部最优 | 1 次 |
| **BFS (ToT)** | 逐层扩展所有候选 | 保证找到最浅的解 | 内存消耗大，不适合深搜索 | O(b^d) |
| **DFS (ToT)** | 深入探索一条路，失败回溯 | 内存高效 | 可能陷入无解的深分支 | O(b*d) |
| **MCTS (LATS)** | UCB1 引导的自适应搜索 | 探索-利用平衡，渐进最优 | 需要大量迭代，成本最高 | O(N * b)，N=迭代次数 |

### 实际应用中的成本-效果权衡

> MCTS 的成本很高——每次迭代都需要多次 LLM 调用。实际使用时需要策略性地控制搜索空间。
