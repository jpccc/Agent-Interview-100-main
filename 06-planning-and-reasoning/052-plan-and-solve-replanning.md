...
**Plan-and-Solve (PS)** 是 Wang et al. (2023, ACL) 提出的零样本 Prompting 策略，核心思想是将任务执行分为“先制定计划、再逐步执行”两个阶段，解决了 Zero-shot CoT 的计算错误、遗漏步骤和语义理解错误三大问题。增强版 PS+ 加入了“提取变量”、“计算中间结果”、“检查计算”等指令，在多个数学推理基准上接近 Few-shot CoT——而无需提供任何示例。

**在 Agent 系统中的演化：** PS 演化为 Plan-and-Execute 架构——Planner Agent 生成全局计划，Executor Agent 逐步执行。然而初始计划不可能完美，执行中必然遇到意外（工具失败、信息偏离、环境变化），因此需要 **动态重规划(Replanning)** 能力。
...
**前沿框架：** DuSAR 结合子目标导向的全局规划与经验驱动的局部适应；ALAS 引入事务性计划执行实现步骤级回滚与重试。从静态规划到动态调整，形成了完整的 Agent 规划闭环。
...
| 维度 | 策略 1：反应式 | 策略 2：周期性 | 策略 3：ALAS 事务性 |
|---|---|---|---|
| **触发时机** | 步骤失败时立即触发 | 每执行 N 步主动评估 | 每个步骤执行前/后 |
| **核心机制** | 保留已完成进度，重新规划剩余 | LLM 评估进度，按需调整计划 | 快照状态，失败回滚 + 重试 |
| **回滚能力** | ❌ 无（失败步骤的结果已污染上下文） | ❌ 无（只调整未来步骤） | ✅ 有（恢复到步骤执行前） |
| **主动性** | 被动（等待失败） | 主动（定期检查） | 被动 + 主动（验证 + 重试） |
| **适用场景** | 步骤间耦合低、失败可局部修复 | 长链路、中间结果可能改变后续方向 | 状态敏感、单步失败代价高 |
| **核心代价** | 低（仅失败时调用 LLM） | 中（周期性调用 LLM） | 高（每步都需要快照 + 验证） |

> **一句话区分**：反应式是"错了再说"，周期性是"定期体检"，ALAS 是"每步存档、错了读档"。
...
> **深度阅读**：[ALAS 事务性规划深度解析](./052.1-alas-deep-dive.md) — 包含数据库事务类比、三种结局流程图、具体例子、与 DuSAR 的对比。
...
> **深度阅读**：[DuSAR 双策略框架深度解析](./052.2-dusar-deep-dive.md) — 包含两个策略的拆解说明、经验库机制、动态重拆解流程图、与 ALAS 的对比。

### 十一、重规划的关键设计原则

> 重规划不是"推倒重来"，而是一套有纪律的局部修正机制。以下 5 个原则缺一不可：

| # | 原则 | 核心要求 | 反面案例（不遵守会怎样） |
|---|------|---------|---------------------|
| 1 | **最小变更** | 保留仍有效的部分，只改必须改的步骤 | 全部推翻重来，浪费已完成的进度和 token |
| 2 | **上下文传递** | 重规划时必须传入已完成结果 + 失败原因 | LLM 从零规划，不知道当前状态，重复已做过的事 |
| 3 | **防止无限循环** | 设最大重规划次数（如 3 次），超过则报告失败 | 无限重试，Agent 永远无法给出结果 |
| 4 | **失败记忆** | 记录已失败的方案，新计划不得重复 | LLM 每次重规划都生成相同方案，反复踩同一个坑 |
| 5 | **降级策略** | 多次失败后有退路：简化目标 / 求助人类 / 部分完成 | 无路可退，只能彻底失败或无限循环 |
