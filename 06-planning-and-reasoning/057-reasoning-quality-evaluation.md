...
评估 Agent 推理质量需要超越“最终答案是否正确”，深入检查 **推理过程本身**。

**核心评估维度：**
1. **逻辑正确性** — 每步推理是否逻辑自洽
2. **忠实性（Faithfulness）** — 推理链是否真实反映了模型的决策过程（而非事后合理化）
3. **完整性** — 是否覆盖了所有关键推理步骤
4. **效率** — 是否用最少的步骤达到目标

**评估方法分类：** 过程评估（评价每一步推理）和结果评估（只看最终输出），前沿研究表明过程评估更可靠。

**关键工具：**
- **Process Reward Models (PRM)** — 对每步推理打分
- **LLM-as-Judge** — 用 LLM 评价另一个 LLM 的推理
- **Agent 专用基准** — SWE-bench、HumanEval、WebArena 等

> ACL 2025 的 Agent 评估综述提出了二维分类法：评估“什么”（能力维度）× “如何”评估（方法维度）。
