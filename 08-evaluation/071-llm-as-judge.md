...
LLM-as-Judge 是用一个强大的 LLM（如 GPT-4）自动评估另一个 LLM 输出质量的技术，在成本和质量之间取得了最佳平衡。

**两种核心模式：**
- **Pointwise 评分** — 对单个输出按维度打分（如 1-5 分）
- **Pairwise 对比** — 比较两个输出哪个更好

**衡量 Judge 可靠性的正确指标是 Cohen's Kappa（κ）**：顶级 Judge（GPT-4o / Claude Opus）的 κ ≈ 0.78-0.84，逼近人类-人类一致性（κ ≈ 0.80）。

> ⚠️ **常见误区：** 很多文章引用“Judge 与人类一致率 80%+”看似很高，但 raw percent agreement 容易虚高（κ=0.62 也能 >80% 一致率），学术界明确指出应优先报告 Kappa。

**已知系统性偏差：** 位置偏差（倾向于给排在前面的答案更高分）、冗长偏差（偏好更长的回答）、自我偏好（GPT-4 Judge 偏好 GPT-4 的输出）。

**缓解策略：** 交换位置多次评估、提供详细的评分 Rubric、使用多个 Judge 投票、定期用人工标注校准。

**2025 新趋势 Agent-as-Judge：** 用 Agent 代替单纯的 LLM 做评估，Agent 可以执行代码验证、搜索事实等操作来辅助判断。
...
| 偏差类型                       | 现象                                    | 缓解策略                                     |
| ------------------------------ | --------------------------------------- | -------------------------------------------- |
| **位置偏差** (Position Bias)   | Pairwise 中倾向于给排在前面的答案更高分 | 交换 A/B 位置做两次评估，取平均或一致性结果  |
| **冗长偏差** (Verbosity Bias)  | 倾向于给更长、更详细的回答更高分        | 在 Rubric 中明确"简洁也是优点"               |
| **自我偏好** (Self-Preference) | GPT-4 做 Judge 时偏好 GPT-4 生成的内容  | 用与被评估模型不同家族的模型做 Judge         |
| **格式偏差** (Format Bias)     | 偏好格式更好（如有列表、标题）的回答    | 在评估时统一格式，或在 Rubric 中降低格式权重 |
| **锚定效应** (Anchoring)       | 提供参考答案时 Judge 被参考答案锚定     | 不提供参考答案做 reference-free 评估         |
...
> 好的 Rubric 是 LLM Judge 可靠性的关键。具体的 Rubric 比模糊的“评估质量”可靠得多。
