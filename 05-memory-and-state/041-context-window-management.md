...
上下文窗口是 LLM 的“工作记忆”——所有输入（System Prompt、对话历史、检索内容、工具结果）和输出都必须装进这个固定大小的 token 空间（4K 到百万级 tokens，如 Claude 标准为 200K，Sonnet 4.6+ / Opus 4.6+ / Mythos 已支持 1M；Gemini 2.5 Pro 也支持 1M+）。

**管理策略（从简到繁）：**
- **截断（Truncation）** — 直接裁剪最早的消息
- **滑动窗口（Sliding Window）** — 只保留最近 N 轮对话
- **观察掩码（Observation Masking）** — 用占位符替代旧内容
- **LLM 摘要（Summarization）** — 用模型将旧对话压缩为摘要
- **分层摘要（Hierarchical）** — 越旧的信息压缩越激进
- **结构化提取** — 提取实体和事实到结构化格式

**研究发现：** 简单策略（掩码、截断）在总效率上往往不逊于复杂的 LLM 摘要。生产最佳实践是将上下文控制在窗口的 75% 以内（用 ACON 等自适应压缩框架做触发式压缩），并注意“中间丢失”效应。

## 详细解析

### 一、为什么需要管理上下文窗口？

上下文窗口 = LLM 能"看到"的全部信息。以下是 128K 窗口的典型 token 分布：
...
**核心思想：** 把窗口外的旧工具结果和长回复替换为一行占位符，省 token。消息的角色和顺序保留，只“遮住”内容——如果后续需要，Agent 可以重新调用工具获取。

**具体例子：**

| 消息 | 掩码前 | 掩码后 |
|------|---------|--------|
| [用户] 帮我查订单 #12345 | 保留 | 保留 |
| [助手] 正在查询… | 保留 | 保留 |
| [工具] 订单详情 JSON | `{"order_id":"12345","status":"shipped","tracking":"SF123456","origin":"上海",... 500字}` | `[内容已省略——如需详情请重新查询]` |
| [用户] 那 #67890 呢？ | 保留 | 保留 |
| [工具] 订单详情 JSON | `{"order_id":"67890","status":"processing",... 又一大段}` | `[内容已省略——如需详情请重新查询]` |
| [用户] 两个订单能合并配送吗？ | 保留（最近消息） | 保留（最近消息） |

> **为什么有效：** 工具返回的 JSON 往往很长（几百甚至上千 token），但对当前问题可能已经没用了。用一行占位符替掉，能省下大量 token 预算给新对话使用。
...
> LLM 对上下文中不同位置的注意力呈 **U 型曲线**：
> - **开头**（System Prompt）→ 注意力高
> - **中间**（参考资料、历史上下文）→ 注意力低，容易被忽略
> - **结尾**（当前用户消息）→ 注意力高
