...
Agent 长期记忆有三种主要存储介质，各撞其长：

1. **向量记忆**（Embedding + 向量数据库）— 擅长语义相似度检索，基于“意思相近”找到相关记忆，适合非结构化文本和模糊查询
2. **结构化记忆**（SQL / NoSQL）— 擅长精确属性查询，适合用户画像、偏好、配置等键值化数据
3. **知识图谱**（Neo4j + Graphiti 等）— 擅长关系推理与时间推理，支持多跳推理（A→B→C）、时间有效性追踪、实体关系网络

**代表性框架 Graphiti（Zep 出品）：** 采用双时间线模型（事件时间 + 入库时间）+ 混合检索（语义 Embedding + 关键词 BM25 + 图遍历）。

**生产推荐：** 混合架构——向量存对话历史和非结构化内容，图谱存事实、实体和关系，结构化存属性配置。Mem0 的混合方案实现了 26% 准确率提升和 90%+ token 成本降低。
