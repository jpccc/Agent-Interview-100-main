...
LLM Agent 的记忆系统借鉴认知科学，分为三种核心类型：

- **短期记忆（Short-Term Memory）** — 当前会话的对话历史，存在于 LLM 的上下文窗口中，会话结束即丢失
- **长期记忆（Long-Term Memory）** — 持久化存储在外部系统（向量数据库、关系数据库）中的知识，跨会话保留，需要检索才能使用
- **工作记忆（Working Memory）** — Agent 在当前任务中主动维护的关键信息子集，本质上就是上下文窗口中当前可用的信息

> **关键理解：** LLM 本身是无状态的，所有“记忆”都是工程层面的外部实现。
